...
async function completeCloseTicket(interaction, meta, pending, confirmedByText) {
  pendingTicketClosures.delete(interaction.channel.id);

  const closingEmbed = new EmbedBuilder()
    .setTitle('🔒 Ticket Closing')
    .setDescription(confirmedByText)
    .setColor('#ED4245')
    .setFooter({
...
async function closeTicket(interaction, reason) {
  ...
  /*
     Custom tickets close immediately.
     They do NOT show the normal ticket closure request because the
     custom-ticket flow is already separate from the normal ticket system.
  */
  if (meta.isCustom) {
    const pending = {
      reason: reason || null,
    ...
    return completeCloseTicket(
      interaction,
      meta,
      pending,
      `This custom ticket was closed by ${interaction.user}.`
    );
  }
